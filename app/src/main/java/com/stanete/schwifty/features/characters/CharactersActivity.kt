...
import com.stanete.schwifty.core.BaseActivity
import com.stanete.schwifty.core.extension.invisible
import com.stanete.schwifty.core.extension.map
...
import com.stanete.schwifty.features.characters.renderers.CharacterRenderable
import com.stanete.schwifty.features.characters.renderers.CharacterRenderer
import com.stanete.schwifty.features.characters.renderers.LoadingRenderable
import com.stanete.schwifty.features.characters.renderers.LoadingRenderer
...
class CharactersActivity : BaseActivity() {
  ...
  private val adapter: RVRendererAdapter<Renderable> by lazy {
    ...
    rendererBuilder.bind(CharacterRenderable::class.java, CharacterRenderer())
    rendererBuilder.bind(LoadingRenderable::class.java, LoadingRenderer(viewModel))
  ...
  private fun renderCharacters(characters: Characters) {
    ...
    adapter.clear()
    adapter.addAll(characters.results.map { CharacterRenderable(it) })
    adapter.add(characters.info.map { LoadingRenderable() })
